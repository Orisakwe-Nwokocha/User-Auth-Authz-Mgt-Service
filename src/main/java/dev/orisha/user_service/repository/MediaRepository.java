...
public interface MediaRepository
  ...
  @Query(
    value = "select media from Media media left join fetch media.uploader",
    countQuery = "select count(media) from Media media"
  )
  Page<Media> findAllWithToOneRelationships(Pageable pageable);

  @Query("select media from Media media left join fetch media.uploader")
  List<Media> findAllWithToOneRelationships();

  @Query(
    "select media from Media media left join fetch media.uploader where media.id =:id"
  )
